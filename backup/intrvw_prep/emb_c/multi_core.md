...
A UDP socket is like a small mailbox identified by: [IP address + port number]
UDP sends one message packet at a time. There is no connection setup, no guarantee of delivery, no retry, and no strict ordering guarantee. Think of it like sending postcards: fast and simple, but the sender does not automatically know if the postcard arrived.
In nr_ue_phy, UE PHY listens on UDP port 12378
...
```
**puts message in PDCCH's queue, increments its semaphore → PDSCH wakes up**
